...
import { submitCodeAndCheckResult,checkSubmissionResult } from "./Controllers/judge0.js";
...
io.on("connection", (socket) => {
  ...
  socket.on("createRoom", ({ roomId, userName }) => {
    if (!rooms[roomId]) {
      ...
      console.log(`Room ${roomId} created and joined by ${userName} (${socket.id})`);
  ...
  socket.on("joinRoom", ({ roomId, userName }) => {
    if (rooms[roomId]) {
      if (rooms[roomId].players.length < 2) {
        ...
        socket.to(roomId).emit("playerJoined", { playerId: socket.id, userName });
        if (rooms[roomId].players.length === 2) {
          io.to(roomId).emit("startGame");
        }
      } else {
        socket.emit("roomJoined", { success: false, message: "Room is full. Cannot join." });
      }
    } else {
      socket.emit("roomJoined", { success: false, message: "Room does not exist" });
    }
  });
  socket.on("submitCode", async ({ problem_id, source_code, language_id, roomId, userName }) => {
    const room = rooms[roomId];

    if (!problem_id || !source_code || !language_id) {
      socket.emit("error", { message: "Missing problem_id, source_code, or language_id" });
      return;
    }
    ...
    try {
      ...
      if (allPassed && room && !room.winner) {
        ...
        io.to(roomId).emit("gameResult", { winner: room.winner, message: `${userName} won the game!` });
      } else {
