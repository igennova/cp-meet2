...
const getcode = async (req, res) => {
    ...
    if (!problem_id || !source_code || !language_id) {
        return res.status(400).json({ message: "Missing problem_id, source_code, or language_id" });
    }
    ...
    try {
        ...
        const results = await Promise.all(submissions.map(submission => submitCodeAndCheckResult(submission)));
...
const checkSubmissionResult = async (submissionId) => {
    ...
    try {
        ...
        while (true) {
            ...
            } else if (statusId === 5) { // Wrong Answer
            ...
            } else if (statusId === 6) { // Time Limit Exceeded
                console.log('Time Limit Exceeded:', resultData);
                return { status: 'Time Limit Exceeded' };
            } else if (statusId >= 7) { // Other errors
                console.log('Submission failed with status:', resultData.status.description);
